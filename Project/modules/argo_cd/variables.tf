...
variable "application_values" {
  description = <<-EOT
    Значення, які Argo CD накладає поверх values.yaml чарта під час рендерингу
    (`spec.source.helm.valuesObject`). Сюди їдуть координати, яких у Git бути не
    може, бо вони існують лише після apply: endpoint RDS, ім'я створеного
    Secret тощо. Файл у Git при цьому лишається джерелом правди для всього
    решти — зокрема для image.tag, який оновлює Jenkins.

    null — жодних перекриттів, чарт розгортається як є.
  EOT
  type        = any
  default     = null
}
