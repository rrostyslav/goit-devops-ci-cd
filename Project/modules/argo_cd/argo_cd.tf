      ...
      applications = [
        {
          ...
          # `helm: null` шаблон пропускає (`{{- with .helm }}`), тож коли
          # перекриттів немає, Application виходить рівно таким, як раніше.
          helm = var.application_values == null ? null : {
            valuesObject = var.application_values
          }
        }
      ]
    })
