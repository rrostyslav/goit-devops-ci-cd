...
output "port_forward_command" {
  description = "Доступ до UI без LoadBalancer — далі https://localhost:8081 (самопідписаний сертифікат)"
  value       = "kubectl port-forward svc/${var.release_name}-server 8081:443 -n ${var.namespace}"
}
