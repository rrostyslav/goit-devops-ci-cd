...
# --- База даних (тема 10) ---
# Модуль підключено через count, тому `one(...)`: коли create_database = false,
# усі виводи нижче стають null, а не ламають план.

output "rds_is_aurora" {
  description = "Що саме створено: true — Aurora-кластер, false — звичайна RDS-інстанція"
  value       = one(module.rds[*].is_aurora)
}

output "rds_endpoint" {
  description = "Хост БД — значення для POSTGRES_HOST у values.yaml чарта"
...
output "rds_port" {
  description = "Порт БД — 5432 для PostgreSQL, 3306 для MySQL"
...
output "rds_master_username" {
  ...
  value       = one(module.rds[*].master_username)
}

output "rds_password_command" {
  description = "Команда, що друкує згенерований AWS пароль до БД"
  value       = one(module.rds[*].password_command)
}

output "rds_connection_command" {
  description = "Готова команда підключення до БД зсередини VPC"
  value       = one(module.rds[*].connection_command)
}
...
output "rds_parameter_group_name" {
  ...
  value       = one(module.rds[*].parameter_group_name)
}

output "rds_parameter_group_family" {
  description = "Родина parameter group, яку модуль обчислив з engine та engine_version"
  value       = one(module.rds[*].parameter_group_family)
}

output "rds_instance_class" {
  description = "Клас інстанса, що фактично застосовано"
  value       = one(module.rds[*].instance_class)
}

output "rds_applied_parameters" {
  description = "Параметри, що потрапили в parameter group. Набір залежить від рушія: у MySQL немає log_statement і work_mem"
  value       = one(module.rds[*].applied_parameters)
}

output "rds_chart_values" {
  description = "Готовий фрагмент для charts/django-app/values.yaml, щоб перемкнути застосунок з вбудованого PostgreSQL на цю базу"
  value = length(module.rds) == 0 ? null : {
    "config.POSTGRES_HOST" = module.rds[0].endpoint
    "config.POSTGRES_PORT" = tostring(module.rds[0].port)
    "config.POSTGRES_DB"   = module.rds[0].database_name
    "config.POSTGRES_USER" = module.rds[0].master_username
